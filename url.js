...
// 設定 Google Form 問卷網址 
const urls = [ ‘https://docs.google.com/forms/d/e/1FAIpQLSdNYHLOrJWbOCkHNM6K9v2GmDwYJwvfPQhd2lLsfgsfg205XQ/viewform?usp=header’, 
...
// 設定最低分配數量和總回應數 
const minCount = 20; 
const totalResponses = 600; // 假設總共需要 600 份問卷 
let distribution = {}; 
let finalList = [];
...
function shuffleArray(array) { 
    for (let i = array.length - 1; i > 0; i–) { 
        ...
        [array[i], array[j]] = [array[j], array[i]]; } } shuffleArray(finalList);

// 4. 輸出結果 console.log(“隨機分配後的 600 份問卷清單：”, 
finalList); console.log(“最終分配統計：”, distribution);

// 5. 提供隨機問卷連結函數（用於網頁應用） 
function getRandomFormLink() { 
    return finalList.pop(); // 取出一個問卷，確保不會重複給到 }
// 如果在網頁應用，可用 
    window.location.href = getRandomFormLink();
